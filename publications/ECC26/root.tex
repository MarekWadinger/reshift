...
\textbf{Truncated online DMD with control (toDMDc):} While the individual components—online DMDc~\cite{Zhang2019} and online SVD~\cite{Zhang2022}—exist separately, our contribution is their integration with basis-alignment updates and a CPD framework. The resulting toDMDc maintains numerical stability while adapting to time-varying dynamics.
...
\end{equation}
where \(r \leq m\) is the truncation rank. Eigendecomposition \(\tilde{A} W = W \Lambda\) yields DMD eigenvalues \(\Lambda = \mathrm{diag}(\lambda_1, \ldots, \lambda_r)\) and eigenvectors \(W\), from which full-dimensional DMD modes are reconstructed as:
\begin{equation}
    \Phi = X' V \Sigma^{-1} W \in \mathbb{R}^{m \times r}.
\end{equation}

Each DMD mode \(\phi_i\) oscillates with frequency and growth rate encoded in the corresponding eigenvalue \(\lambda_i = \sigma_i + i\omega_i\), where \(\sigma_i\) indicates growth/decay and \(\omega_i\) the oscillation frequency~\cite{Brunton2022}.
...
Since \(B\) is typically unknown, we augment the state with control inputs~\cite{Proctor2016}:
\begin{equation}\label{eq:augmented}
    \bar{X} = \begin{bmatrix} X \\ \Theta \end{bmatrix} \in \mathbb{R}^{(m+l) \times n},
\end{equation}
and solve for the augmented operator \(\bar{A} = [A \mid B]\) that maps \(\bar{X}\) to \(X'\). When \(B\) is known, one can instead compensate \(\bar{X}' = X' - B\Theta\) and apply standard DMD.
...
\begin{align}
    P_{k+c} & = P_k - P_k X_{k:k+c} \Gamma_{k:k+c} X_{k:k+c}^\top P_k, \label{eq:p-update}                \\
    A_{k+c} & = A_k + (X'_{k:k+c} - A_k X_{k:k+c}) \Gamma_{k:k+c} X_{k:k+c}^\top P_k, \label{eq:a-update}
\end{align}
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
where \(p\) and \(q\) are the ranks of the state and control subspaces. Since the online SVD update changes the basis \(\tilde{U}_k\), the existing reduced-order matrices must be re-expressed in the new coordinates. The reduced-order matrices are then transformed:
\begin{align}
...
\end{align}
Note that \(K^{U'U}_k\) is well-conditioned when the truncation rank \(r \leq \min(m, l)\), as it is a submatrix of a unitary rotation; near-singularity would indicate an abrupt subspace change, which is itself a detectable event.
...
We maintain three sliding windows, ordered chronologically as learning, base, and test:
\begin{itemize}
    \item \textbf{Learning window} \(\ui{\bar{X}}{L}\): Oldest \(d\) samples used for model updates
    \item \textbf{Base window} \(\ui{\bar{X}}{B}\): Next \(a\) samples representing the reference behavior, separated from the test window by \(b\) samples
    \item \textbf{Test window} \(\ui{\bar{X}}{T}\): Most recent \(c\) samples being evaluated for changes
\end{itemize}
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE Revert old snapshots via~\eqref{eq:p-update}--\eqref{eq:a-update} with \(-C\)
        \ENDIF
...
\textbf{Detection delay:} The peak of \(Q_k\) occurs approximately \(c\) samples after the true change-point, as the test window must fully contain the changed data. This provides a deterministic lower bound on detection delay, with the actual delay depending on noise level and change magnitude.

\textbf{Convergence:} Under stationarity with bounded eigenvalues \(\lambda_{\min}(X_k X_k^\top) > 0\), the online updates~\eqref{eq:p-update}--\eqref{eq:a-update} converge to the batch DMD solution, with convergence rate governed by the condition number of \(X_k X_k^\top\)~\cite{Zhang2019}. The truncated SVD further ensures numerical stability by preventing small singular values from corrupting \(P_k\).

\textbf{Interpretability:} The detection statistic \(Q_k\) reflects the \textit{relative} dissimilarity between base and test data in the learned subspace. While \(Q_k\) does not have an absolute scale (its magnitude depends on the operating regime), it provides a consistent ranking of change severity. In practice, calibration via historical \(Q_k\) values during normal operation is recommended for threshold selection.
...
\section{Numerical Validation}\label{sec:results}

We validate the framework on two simulation studies and one real-data case study, representing different change types, system complexities, and industrial scenarios. Table~\ref{tab:experiments} summarizes the configurations and key characteristics.
...
\begin{figure}[ht]
    ...
    \caption{Detection of synthetic step changes. The signal (top) contains nine steps of increasing magnitude. The ODMD-CPD statistic (middle two plots) peaks close to 100 samples after each true change-point. Reference SVD-based method (bottom) shows higher noise, potentially masking first two subtle changes.}\label{fig:synthetic}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{CPD on nonlinear two-tank system with input delay. System states (top) and control input (second) show complex dynamics. The ODMD-CPD statistic (third) detects sensor bias, control response change, and linear drift with clear peaks. Reference method (bottom) has noisier statistics.}\label{fig:twotank}
\end{figure}

\subsection{Industrial Battery Energy Storage System (BESS)}
...
\begin{figure}[ht]
    ...
    \caption{Faulty cooling system operation in BESS. Temperature profiles (top) show gradual elevation after the fault. The ODMD-CPD statistic (middle) detects the transition with early precursor signals. Error difference formulation (bottom) shows bidirectional detection (positive peaks for transitions to fault, negative for returns to normal).}\label{fig:bess}
\end{figure}
...
Compared to statistical CPD methods (CUSUM, GLR)~\cite{Ye2023}, ODMD-CPD requires no distributional assumptions and explicitly models temporal dynamics. Unlike deep learning approaches such as TIRE~\cite{DeRyck2021}, our method is fully unsupervised and provides interpretable statistics through reconstruction error ratios, with DMD modes enabling root cause analysis via dominant frequencies and growth rates. Relative to subspace methods~\cite{Moskvina2003,Kawahara2007}, ODMD-CPD explicitly handles control inputs, disambiguating actuation effects from system changes. Compared to kernel methods~\cite{Ferrari2023,Chu2022}, our approach scales as \(\mathcal{O}(mr^2)\) versus \(\mathcal{O}(n^2)\) and yields more interpretable statistics.

\subsection{Practical Deployment Considerations}

While we used fixed \(\tau\) in the experiments, adaptive thresholding based on historical \(Q_k\) distribution (e.g., \(\tau = \text{95th percentile of }Q_k\) during normal operation) can reduce false alarms at the cost of potential systematic bias. Using multiple test window sizes enables multi-resolution detection: fast windows for abrupt changes, slow windows for gradual drift. Furthermore, the reconstruction error can be decomposed per DMD mode to identify \textit{which} dynamics changed, supporting root cause analysis.
