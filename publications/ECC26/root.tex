...
\begin{equation}\label{eq:detection-guarantee}
...
All experiments use rank truncation (\(r=2\)--10) and windowed online updates with forgetting factors to adapt to time-varying conditions. Throughout this section, figures adopt a common convention: solid vertical lines mark the true occurrence of each change-point at time \(t_c\), while dashed red vertical lines mark the probabilistic detection bound \( t_c + c + \delta \) from~\eqref{eq:detection-guarantee}, within which \(Q_k\) is expected to exceed the threshold \( \tau \) with probability at least \( 1-\alpha \). The horizontal axis in each Figure
...
Unlike statistical methods (CUSUM, GLR)~\cite{Ye2023} that assume a known distribution and fail under temporal correlation, ODMD-CPD makes no such assumptions and explicitly models dynamics. Deep-learning approaches like TIRE~\cite{DeRyck2021} achieve high accuracy but lack interpretability and require labeled data; ODMD-CPD is unsupervised, with interpretable reconstruction-error statistics and DMD modes that enable root cause analysis. Autonomous subspace methods~\cite{Moskvina2003, Kawahara2007} ignore control inputs, whereas our DMDc formulation disambiguates actuation from system change. Kernel methods~\cite{Ferrari2023, Chu2022} scale as \(\mathcal{O}(n^2)\) versus our \(\mathcal{O}(mr^2)\) and are less interpretable.
